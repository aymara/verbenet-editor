...
class VerbnetReader:
    ...
    def __init__(self, path, normalize = True):
        ...
        for filename in os.listdir(path):
            ...
            self.files[filename[:-4]] = self._handle_class(root.getroot(), [], set())
    ...
    def _handle_class(self, xml_class, parent_frames, role_list):
        ...
        """
        frames = []
        role_list = copy.deepcopy(role_list)
        ...
        for xml_role in xml_class.find("THEMROLES"):
            role_list.add(xml_role.attrib["type"])
        whole_class["roles"] = role_list

        for xml_frame in xml_class.find("FRAMES"):
            frames.append(self._build_frame(xml_frame, vnclass, role_list))
        whole_class["frames"] = frames
        
        for xml_verb in xml_class.find("MEMBERS"):
            verb = xml_verb.attrib["name"]
            whole_class["members"].append(verb)
        ...
        for subclass in xml_class.find("SUBCLASSES"):
            whole_class["children"].append(self._handle_class(subclass, frames, role_list))
    ...
    def _build_structure(self, base_structure, syntax_data, vnclass, role_list):
        ...
        for i, full_element in enumerate(base_structure):
            ...
            elif self.normalize and element == "PP":
